...
// Description: Verilog module for PRBS31 generator,this module is 12 times shift result.
// Polynomial: 1 + x^28 + x^31
// Width: 32
// Iterations: 12
//--------------------------------------------------------------------------------------

module prbs31_width32_iter12 (
    input [31:0] din,
    output [31:0] dout
);

    assign dout[0] = din[16] ^ din[19];
    assign dout[1] = din[17] ^ din[20];
    assign dout[2] = din[18] ^ din[21];
    assign dout[3] = din[19] ^ din[22];
    assign dout[4] = din[20] ^ din[23];
    assign dout[5] = din[21] ^ din[24];
    assign dout[6] = din[22] ^ din[25];
    assign dout[7] = din[23] ^ din[26];
    assign dout[8] = din[24] ^ din[27];
    assign dout[9] = din[25] ^ din[28];
    assign dout[10] = din[26] ^ din[29];
    assign dout[11] = din[27] ^ din[30];
