...
// Description: Verilog module for PRBS15 generator,this module is 12 times shift result.
// Polynomial: 1 + x^14 + x^15
// Width: 32
// Iterations: 12
//--------------------------------------------------------------------------------------

module prbs15_width32_iter12 (
    input [31:0] din,
    output [31:0] dout
);

    assign dout[0] = din[2] ^ din[3];
    assign dout[1] = din[3] ^ din[4];
    assign dout[2] = din[4] ^ din[5];
    assign dout[3] = din[5] ^ din[6];
    assign dout[4] = din[6] ^ din[7];
    assign dout[5] = din[7] ^ din[8];
    assign dout[6] = din[8] ^ din[9];
    assign dout[7] = din[9] ^ din[10];
    assign dout[8] = din[10] ^ din[11];
    assign dout[9] = din[11] ^ din[12];
    assign dout[10] = din[12] ^ din[13];
    assign dout[11] = din[13] ^ din[14];
