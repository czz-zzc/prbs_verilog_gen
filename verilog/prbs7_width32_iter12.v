...
// Description: Verilog module for PRBS7 generator,this module is 12 times shift result.
// Polynomial: 1 + x^6 + x^7
// Width: 32
// Iterations: 12
//--------------------------------------------------------------------------------------

module prbs7_width32_iter12 (
    input [31:0] din,
    output [31:0] dout
);

    assign dout[0] = din[0] ^ din[2];
    assign dout[1] = din[1] ^ din[3];
    assign dout[2] = din[2] ^ din[4];
    assign dout[3] = din[3] ^ din[5];
    assign dout[4] = din[4] ^ din[6];
    assign dout[5] = din[0] ^ din[5] ^ din[6];
